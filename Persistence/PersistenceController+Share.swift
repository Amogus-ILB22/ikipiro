...
//    }
    func presentCloudSharingController(toko: Toko, name: String) {
        /**
         Grab the share if the photo is already shared.
         */
        var tokoShare: CKShare?
        if let shareSet = try? persistentContainer.fetchShares(matching: [toko.objectID]),
           let (_, share) = shareSet.first {
            tokoShare = share
        }
        ...
        let sharingController: UICloudSharingController
        if tokoShare == nil {
            sharingController = newSharingController(unsharedToko: toko, persistenceController: self)
        } else {
...
//    }
    ...
    private func newSharingController(unsharedToko: Toko, persistenceController: PersistenceController) -> UICloudSharingController {
        return UICloudSharingController { (_, completion: @escaping (CKShare?, CKContainer?, Error?) -> Void) in
            /**
             The app doesn't specify a share intentionally, so Core Data creates a new share (zone).
             CloudKit has a limit on how many zones a database can have, so this app provides an option for users to use an existing share.
             ...
             The default value of publicPermission is CKShare.ParticipantPermission.none.
             */
            self.persistentContainer.share([unsharedToko], to: nil) { objectIDs, share, container, error in
                if let share = share {
                    self.configure(share: share)
                }
                completion(share, container, error)
